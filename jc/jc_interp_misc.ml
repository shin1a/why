...
let alloc_table_detailed_writes ~mode ~type_safe ~callee_writes ?region_assoc ~region_mem_assoc () =
...
let alloc_table_writes ~mode ~type_safe ~callee_writes ?region_assoc
    ~region_mem_assoc () =
  List.map snd (alloc_table_detailed_writes ~mode ~type_safe ~callee_writes ?region_assoc ~region_mem_assoc ())
    ...
    AllocMap.fold
	 ...
	 if has_alloc_table (ac,distr) callee_writes.jc_effect_alloc_tables then
	   ...
	   match mode with
	     | #param_or_local_mode -> acc
	     | #effect_mode ->
		 add_alloc_table_argument ~mode ~type_safe ~no_deref:false (ac,distr) ?region_assoc acc ()
...
let memory_detailed_writes ~mode ~type_safe ~callee_writes ?region_assoc ~region_mem_assoc () =
...
let memory_writes ~mode ~type_safe ~callee_writes ?region_assoc ~region_mem_assoc () = 
  List.map snd
...
let memory_reads ~mode ~type_safe ~callee_writes ~callee_reads ?region_assoc ~region_mem_assoc ~already_used () = 
  List.map snd
