...
module TermOrd = struct
  ...
  let rec compare t1 t2 =
    match t1#node, t2#node with
      ...
      | JCTvar v1,JCTvar v2 ->
	  Stdlib.compare v1.jc_var_info_tag v2.jc_var_info_tag
      | JCTbinary(t11,op1,t12),JCTbinary(t21,op2,t22) ->
	  let compop = Stdlib.compare op1 op2 in
	  if compop = 0 then
	  ...
	  let compabs = Stdlib.compare absolute1 absolute2 in
	  if compabs = 0 then
	    compare t11 t21
	  else compabs
      | JCTderef(t11,_,fi1),JCTderef(t21,_,fi2) ->
	  let compfi =
	    Stdlib.compare fi1.jc_field_info_tag fi2.jc_field_info_tag
	  in
	  if compfi = 0 then compare t11 t21 else compfi
      | JCToffset(ok1,t11,st1),JCToffset(ok2,t21,st2) ->
	  let compok = Stdlib.compare ok1 ok2 in
	  if compok = 0 then
	    let compst =
	      Stdlib.compare st1.jc_struct_info_name st2.jc_struct_info_name
	    in
	  ...
	  let compli =
	    Stdlib.compare li1.jc_logic_info_tag li2.jc_logic_info_tag
	  in
	  if compli = 0 then
  ...
  let rec compare a1 a2 =
    match a1#node, a2#node with
      ...
      | JCAquantifier(q1,vi1,trigs1,a1),JCAquantifier(q2,vi2,trigs2,a2) ->
          let compq = Stdlib.compare q1 q2 in
          if compq = 0 then
	    let compvi = Stdlib.compare vi1 vi2 in
	    if compvi = 0 then
      ...
      | JCAeqtype(tag11,tag12,so1),JCAeqtype(tag21,tag22,so2) ->
          let compso = option_compare Stdlib.compare so1 so2 in
          if compso = 0 then
